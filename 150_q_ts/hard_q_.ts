...
function mergeTwo(list1: ListNode | null, list2: ListNode | null): ListNode | null {
...
function mergeListHelper(lists: Array<ListNode | null>, left: number, right: number): ListNode | null {
...
function findWords(board: string[][], words: string[]): string[] {
    ...
    // DFS для поиска слов
    function findWord(i: number, j: number, cur: TrieNode) {
        const ch = board[i][j];
        if (cur.nxt.has(ch)) {
